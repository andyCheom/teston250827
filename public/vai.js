document.addEventListener('DOMContentLoaded', () => {
    ...
    const submitButton = promptForm.querySelector('button[type="submit"], input[type="submit"]');
    ...
    if (!chatContainer || !promptForm || !promptInput || !imageInput) {
        console.error("Essential chat components not found in the DOM. Aborting script.");
        document.body.innerHTML = '<p style="color: red; text-align: center; margin-top: 2rem;">채팅 인터페이스를 불러오는 데 실패했습니다. 페이지의 HTML 요소를 확인해주세요.</p>';
        return;
    }
    ...
    async function handleFormSubmit(e) {
        ...
        try {
            ...
            if (!Array.isArray(conversationHistory)) {
                console.warn('conversationHistory가 배열이 아니므로 초기화합니다.', conversationHistory);
                conversationHistory = [];
            }
            historyString = JSON.stringify(conversationHistory);
        } catch (error) {
            console.error("conversationHistory를 JSON 문자열로 변환하는 데 실패했습니다. 비어있는 기록을 전송합니다.", error);
        ...
        try {
            ...
            if (Array.isArray(result.updatedHistory)) {
                ...
                console.warn("Server returned invalid 'updatedHistory'. History will not be updated for this turn.");
            }
            
            const modelResponseText = result.vertexAiResponse?.candidates?.[0]?.content?.parts?.[0]?.text;
            ...
            if (error instanceof TypeError) {
                ...
                console.error("개발자 정보: TypeError가 발생했습니다. 네트워크 실패(CORS, DNS, 서버 다운)일 가능성이 높습니다.");
                ...
                console.error("개발자 정보: SyntaxError가 발생했습니다. 서버 응답이 유효한 JSON이 아닙니다. 서버 로그에서 HTML 오류 페이지를 반환하는 충돌이 있는지 확인하세요.");
                userMessage = '서버로부터 잘못된 형식의 응답을 받았습니다. 서버에 문제가 있을 수 있습니다.';
            } else {
                // `!response.ok` 블록에서 발생시킨 사용자 정의 오류일 가능성이 높습니다.
                console.error(`개발자 정보: 처리된 API 오류입니다. 메시지: "${error.message}"`);
